...
/**
 * Sequential (textbook) epsilon-constraint search over the 7 soft-criteria
 * priority tiers, layered on top of - not a replacement for - big-M
 * dominance weighting (GenerationConfig::tierWeight()): instead of one
 * SimulatedAnnealingOptimizer pass whose objective big-M-weights ALL 7
 * tiers for the entire search, this runs one pass per tier, where pass m's
 * objective still big-M-weights every tier from m onward exactly as before
 * (tier m dominates the search, m+1..6 ride along as free tie-breaks) - the
 ...
 * never allows.
 *
 * "Tier" here is always a rank position in $config->softCriteria, not a
 * fixed criterion key - since softCriteria is configurable per Association,
 * whichever criterion currently occupies rank 1 is what gets optimized (and
 * fixed) first, exactly as GenerationConfig::tierWeight() already resolves
 * rank from the same array.
 */
final class EpsilonConstraintOptimizer
{
    ...
    public function optimize(
        ...
        $tierCount = count($config->softCriteria);
        ...
        for ($m = 0; $m < $tierCount; $m++) {
            $excluded = array_slice($config->softCriteria, 0, $m);
