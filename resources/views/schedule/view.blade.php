            ...
            <?php if (!$schedule->rounds->isEmpty()): ?>
                ...
                <?php foreach ($schedule->rounds->sortBy('start_date') as $round): ?>
                    ...
                    <a class="list-group-item list-group-item-action" href="{{ route('round.edit', ['association' => $schedule->association, 'schedule' => $schedule, 'round' => $round]) }}">
                        <?php echo ('<div class="round">' . ($round->off_week ? '<em>' : '') . (!empty($round->name) ? ($round->name . ' - ') : '') . (!empty($round->start_date) ? $round->start_date->format('m-d-Y') : '') . ($round->off_week ? ' - Off Week</em>' : ($scheduledMatchCount > 0 ? ' - ' . $scheduledMatchCount . ' Matches' : ' - No Matches')) . '</div>'); ?>
                    </a>
                <?php endforeach; ?>
                </div>
            <?php else: ?>
