...
if [ "$CONFIG" != "local" ]; then
  rsync -rvz --delete --filter='protect bootstrap/cache/***' --exclude='public/hot' --files-from "deploy.files" . "${USER}@${HOST}:${DOCROOT}"
else
  rsync -rvz --delete --filter='protect bootstrap/cache/***' --exclude='public/hot' --files-from "deploy.files" . "${DOCROOT}"
fi
